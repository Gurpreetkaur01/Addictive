...
class AppDelegate: UIResponder, UIApplicationDelegate {
    ...
    func application(_ application: UIApplication, didFinishLaunchingWithOptions launchOptions: [UIApplicationLaunchOptionsKey: Any]?) -> Bool {
        ...
        UIApplication.shared.statusBarStyle = .lightContent
         ...
         //self.setNotificationFunction(application: application)
        GMSServices.provideAPIKey("AIzaSyDB4Hu14dcQEfVtoL-mUA4XvhdxDxRmVrA")
        GMSPlacesClient.provideAPIKey("AIzaSyDB4Hu14dcQEfVtoL-mUA4XvhdxDxRmVrA")
        
        if  standard.value(forKey: "user_id") as? String != nil {
            self.homeTabBar()
        }
        else{
    ...
    func homeTabBar()
    {
        let vc =  UIStoryboard.init(name: "Main", bundle: nil).instantiateViewController(withIdentifier: "HomeViewController")
         ...
         let vchild =  UIStoryboard.init(name: "Main", bundle: nil).instantiateViewController(withIdentifier: "ChildViewController")
        
        let nav = UINavigationController()
        
        if standard.value(forKey: "user_type") as? String == "parent" {
             nav.viewControllers = [vc]
        }
        else{
            nav.viewControllers = [vchild]
        }
